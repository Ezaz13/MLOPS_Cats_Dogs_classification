...
-   **Workflow Stages**:
    1.  **Environment Setup**: Ubuntu runner with Python 3.9 and dependency installation.
    2.  **Linting**: Uses `flake8` to enforce PEP8 standards (max line length: 127) and check for syntax errors.
    3.  **Unit Testing**: Executes `pytest` on the `tests/` directory to validate code logic.
    4.  **Integration Testing**: Runs the full `pipeline.py` script to ensure the data-to-model flow is unbroken.
    5.  **Artifact Archival**: Uploads build artifacts for review:
        -   `model-performance-report`: Markdown summary of training results.
        -   `mlflow-runs`: Complete MLflow experiment logs.
        -   `validation-reports`: Data quality checks from Great Expectations.

### Deployment
-   **Containerization** (`Dockerfile`):
    -   **Base Image**: `python:3.10-slim` for a lightweight, secure runtime.
